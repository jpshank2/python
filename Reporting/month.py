...
def month(conn, name, wb):
    ws = wb['Sheet']
    ws.title = "Monthly Due"
    rowNum = 2
    col = 1
    cursor = conn.cursor()
	...
	AND T.CurrentDueDate >= '""" + c_year + """-""" + c_month + """-01' AND T.CurrentDueDate <= '""" + c_year + """-""" + n_month + """-01'
	AND (Own.StaffName = '""" + name + """' OR
	Ptnr.StaffName = '""" + name + """' OR
	Mgr.StaffName = '""" + name + """' OR
	Jb.StaffName = '""" + name + """')

...
namerow.execute("""SELECT M.Member_RecID, CONCAT(M.First_Name, ' ', M.Last_Name) AS [Name], M.Email_Address, TEP.Date_Start, TEP.Date_End, TES.Description
FROM dbo.Time_Sheet TS
INNER JOIN dbo.Member M ON M.Member_RecID = TS.Member_RecID
INNER JOIN dbo.TE_Status TES ON TES.TE_Status_ID = TS.TE_Status_ID
INNER JOIN dbo.TE_Period TEP ON TEP.TE_Period_RecID = TS.TE_Period_RecID AND DATEADD(DAY, -4, GETDATE()) BETWEEN TEP.Date_Start AND TEP.Date_End
WHERE TES.TE_Status_ID = 0 AND M.Member_RecID IN (""" + staffIndexForSQL + """)""")
